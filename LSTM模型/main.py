...
class XXL_LSTM(nn.Module):
    def __init__(self, i_s, h_s):
        super().__init__()
        self.i_s = i_s
        self.h_s = h_s
        self.W = nn.Parameter(torch.Tensor(i_s, h_s * 4))
        self.U = nn.Parameter(torch.Tensor(h_s, h_s * 4))
        self.bias = nn.Parameter(torch.Tensor(h_s * 4))
        xav = 1.0 / math.sqrt(self.h_s)
        for weight in self.parameters():
            weight.data.uniform_(-xav, xav)
    ...
    def forward(self, x,
        ...
        h_seq = []
        if ini_s is None:
            h_t, c_t = (torch.zeros(bs, self.h_s).to(x.device),
                        torch.zeros(bs, self.h_s).to(x.device))
        else:
            h_t, c_t = ini_s
        HS = self.h_s
...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        self.LSTM = XXL_LSTM(emb_size, n_hidden)
